...
- [x] G8: rendered in a real browser, no page overflows horizontally at any of 13 widths from 320 to 1920, and every rendered text/background pair meets WCAG AA — including text set over photographs, which is verified against the image's own pixels
  ...
  2026-09-14 re-verification after the phone cover rework: at 375px the hero kicker, headline, gold word, sub line and "how I work" link were measured against the round-8 listening photograph composited with both CSS dissolves (canvas in the pane) — worst case 7.61:1 (sub line), headline 14.5:1. The FAQ intro over the daylight stamp under the paper veil: label 4.91:1, headline 6.78:1. Horizontal overflow 0 at 375px.
  2026-09-14, second re-verification — the portrait retouch commit (4a2dffa) replaced `hero-room`, which the DESKTOP hero sets white text over with no scrim, so the measurement had to be repeated against the new pixels: at 1440px, kicker 6.49:1, headline 11.27:1, gold word 7.72:1, sub line 5.75:1, name 14.46:1 — all above threshold, worst case the sub line. The retouch lifted the frame (files roughly doubled in size), so the headline came down from the 9.74–14.33:1 band to 11.27:1 and the sub line is now the tightest pair. Any further brightening of this photograph needs this measurement re-run before it ships.
  2026-09-14, third re-verification — the round-9 closing photograph (blue hour, no sun) replaced the round-8 one, so the centred closing text was measured over it at 375px. This found a defect that predates round 9 and that the earlier phone sweep had missed, because it measured the hero and the FAQ band but never this one: at the shipped scrim of .74 the round-8 frame put its sunset glow under the sub line at 3.96:1, and the round-9 frame puts a city light under the channel labels at 4.09:1 — different elements, same cause, both below 4.5. The scrim was solved numerically rather than eyeballed: .80 is the shallowest value that clears every pair on the current image (headline 15.50:1, sub line 5.49:1, channel labels 4.91:1, channel values 14.27:1), and the photograph is dark enough (mean luminance 18.4 against the old frame's 25.6) that the silhouette survives it. The gold button is excluded from this sweep by design: it carries its own opaque fill, and ink-on-gold is already gated at the token level — measuring its label against the photograph behind it reports a meaningless 1.06:1.
